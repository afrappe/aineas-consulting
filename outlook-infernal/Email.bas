...
' AINEAS CONSULTING (c) 2024
' Autor: Arturo Frappé Muñoz
' Modulo VBA para procesar correspondencia de Outlook 16
' Requisitos:
...
Private Const SUBJECT_REJECT_PATTERN As String = "Entregado:|Retransmitido:|Leído:|Read:|Aceptada:|Accepted:|Respuesta automática:|Expandido:"
Private Const BODY_CUT_PATTERN As String = "De:"
    ' Configure mailbox
Private Const MAILBOX As String = "mailbox"  'Cambiar este valor
...
Function GetLastDate() As Date
    ...
    Dim lastRow As Long
    With ThisWorkbook.Sheets("Proceso")
...
Function GetRecipientEmail(ByVal Recipient As Object) As String
    ...
    If Err.Number <> 0 Then
      If Recipient.AddressEntry.AddressEntryUserType = olExchangeDistributionListAddressEntry Then
       GetRecipientEmail = Recipient.AddressEntry.GetExchangeDistributionList.PrimarySmtpAddress
      Else
        GetRecipientEmail = ""
   ...
   ' On Error GoTo ErrorHandler
    ...
    For Each mailItem In MFldr.Items.Restrict(sFilter)
        If Not regexSubject.Test(mailItem.Subject) Then
         If mailItem.Class <> olMeetingRequest Then
